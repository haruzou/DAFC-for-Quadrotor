...
for i = 1:n
    ...
    xi = X(i, 1);    yi = X(i, 2);    zi = X(i, 3);
    vxi = X(i, 4);   vyi = X(i, 5);   vzi = X(i, 6);
    
    Theta_xi = X(i, 15:50)';
    Theta_yi = X(i, 51:86)';
    Theta_zi = X(i, 87:122)';
    
    zeta_xi = zeta_p(xi, vxi, c_x, w_x, c_x_dot, w_x_dot);
    zeta_yi = zeta_p(yi, vyi, c_y, w_y, c_y_dot, w_y_dot);
    zeta_zi = zeta_p(zi, vzi, c_z, w_z, c_z_dot, w_z_dot);
    
    uxi = Theta_xi.' * zeta_xi;
    uyi = Theta_yi.' * zeta_yi;
    uzi = Theta_zi.' * zeta_zi;

    Ti = m*sqrt(uxi^2 + uyi^2 + (uzi + g)^2); 
    
    phi_d = asin((uxi*sin(psi_d) - uyi*cos(psi_d)) / sqrt(uxi^2 + uyi^2 + (uzi + g)^2));

    theta_d = atan((uxi*cos(psi_d) + uyi*sin(psi_d)) / (uzi + g));
end
%% =========================================================================
%% 5. PLOTTING RESULTS
%% =========================================================================
...
subplot(3,1,1);
plot(t,phi_d,'r','LineWidth',1.5); hold on;
plot(t,phi,'b--'); grid on
ylabel('\phi [rad]'); legend('\phi_d','\phi','Location','best');
subplot(3,1,2);
plot(t,theta_d,'r','LineWidth',1.5); hold on;
plot(t,theta,'b--'); grid on
ylabel('\theta [rad]'); legend('\theta_d','\theta','Location','best');
subplot(3,1,3);
plot(t,psi_d,'r','LineWidth',1.5); hold on; plot(t,psi,'b--'); grid on
ylabel('\psi [rad]'); xlabel('t [s]'); legend('\psi_d','\psi','Location','best');
%% =========================================================================
%% CONTROL FUNCTION
%% =========================================================================
function deq = f(t, X)  
